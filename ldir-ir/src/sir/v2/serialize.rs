...
pub fn deserialize_module(bytes: &[u8]) -> Result<SIRModuleV2, String> {
    ...
    let patch = bytes[6];
    if major != 2 {
        return Err(format!("unsupported version {}.{}.{}", major, minor, patch));
    }
    ...
    let created = u64::from_le_bytes(
        bytes[9..17]
            .try_into()
            .map_err(|e: core::array::TryFromSliceError| e.to_string())?,
    );

    let mut pos = 17;

    // Source format
    let sf_len = u16::from_le_bytes(
        bytes[pos..pos + 2]
            .try_into()
            .map_err(|e: core::array::TryFromSliceError| e.to_string())?,
    ) as usize;
    ...
    for _ in 0..5 {
        if pos + 4 > bytes.len() {
            return Err("unexpected end of sections".into());
        }
        ...
        if pos + len > bytes.len() {
            return Err("section data truncated".into());
        }
