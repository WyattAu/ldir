...
use crate::compiler::context::{CompileContext, FONT_ID_BOLD, FONT_ID_BOLD_ITALIC, FONT_ID_ITALIC, FONT_ID_REGULAR};
use crate::compiler::emit_helpers;
use crate::compiler::justify;
use crate::error::Result;
use crate::fp266::Fp266;
use crate::layout::linebreak::{linebreak, LineBreakItem, LineBreakOptions};
use crate::layout::linebreak::cjk::{is_cjk_text, insert_cjk_breaks};
...
pub fn compile_v2_document(module: &SIRModuleV2, ctx: &mut CompileContext) -> Result<GIRDocument> {
    ...
    for &root_id in module.body.roots() {
        compile_v2_node(
            root_id, module, &mut page, ctx, &mut gir_doc,
            &mut labels, &refs_map, &mut eq_counter,
            &mut section_counters, &mut section_number,
        )?;
    }
...
pub fn compile_v2_document_with_bib(
    ...
    for &root_id in module.body.roots() {
        compile_v2_node_with_bib(
            ...
            &mut labels, &refs_map, &mut eq_counter,
            &mut section_counters, &mut section_number,
            bibliography, &mut cite_counter, &mut cite_numbers,
        )?;
    }

    emit_bibliography_page(module, bibliography, &cite_numbers, ctx, &mut page, &mut gir_doc);
...
fn resolve_v2_fonts(ctx: &mut CompileContext, resources: &ResourceDecls) {
    ...
    for decl in &resources.fonts {
        let weight = match decl.weight {
            ...
            | ldir_ir::sir::v2::resources::FontWeight::Black => {
                fontdb::Weight::BOLD
            }
            ...
            | ldir_ir::sir::v2::resources::FontWeight::Medium => {
        ...
        let style = match decl.style {
            ...
            | ldir_ir::sir::v2::resources::FontStyle::Oblique => {
                fontdb::Style::Italic
            }
...
fn compile_v2_node(
    ...
    let node = module.body.get(node_id)
        .ok_or(crate::error::LdirError {
            kind: crate::error::ErrorKind::Compile(crate::error::CompileErrorKind::UnsupportedInstruction { entity_id: node_id }),
    ...
    match &node.node_type {
        NodeType::Document => {
            compile_children_with_refs(node_id, module, page, ctx, gir_doc, labels, refs_map, eq_counter, section_counters, section_number)?;
        }
        NodeType::Paragraph => {
        ...
        NodeType::Styled { style_name } => {
            let _style = module.styles.find(style_name);
            compile_children_with_refs(node_id, module, page, ctx, gir_doc, labels, refs_map, eq_counter, section_counters, section_number)?;
        }
        NodeType::BlockQuote => {
        ...
        NodeType::Image { source, .. } => {
            let base_dir = module.header.source_path.as_ref()
                .and_then(|p| std::path::Path::new(p).parent());
            if let Some((img, w_fp, h_fp)) = load_and_scale_image(source, ctx.content_width, base_dir) {
                let image_index = gir_doc.push_image(img);
                emit_helpers::emit_move_xy(page, ctx.x, ctx.y);
                page.push(GIRCommand::new_draw_rule(-1, image_index as i32, w_fp, h_fp));
        ...
        NodeType::Table { .. } => {
            compile_v2_table_fallback(node_id, module, page, ctx, gir_doc)?;
        }
        NodeType::Figure { .. } | NodeType::Caption => {
            compile_children_with_refs(node_id, module, page, ctx, gir_doc, labels, refs_map, eq_counter, section_counters, section_number)?;
        }
        NodeType::Footnote { content } => {
            let display = format!("[{}] {}", '\u{00B9}', content);
            emit_v2_text_inline(&display, page, ctx, gir_doc);
        }
...
fn compile_v2_node_with_bib(
    ...
    match &node.node_type {
        NodeType::Document => {
            compile_children_with_bib(node_id, module, page, ctx, gir_doc, labels, refs_map, eq_counter, section_counters, section_number, bibliography, cite_counter, cite_numbers)?;
        }
        NodeType::Paragraph => {
            emit_v2_paragraph_with_bib(node_id, module, page, ctx, gir_doc, labels, refs_map, bibliography, cite_counter, cite_numbers)?;
        }
        NodeType::Chapter => {
            increment_section_counter(1, section_counters, section_number);
            let num = section_number_string(section_number);
        ...
        NodeType::BlockQuote => {
            ...
            ctx.reset_x();
            let rule_x = saved_margin + Fp266::from_int(12);
            emit_helpers::emit_draw_rule(page, rule_x, ctx.y, Fp266::from_int(2), ctx.line_height() * 2);
            compile_children_with_bib(node_id, module, page, ctx, gir_doc, labels, refs_map, eq_counter, section_counters, section_number, bibliography, cite_counter, cite_numbers)?;
            ctx.margin_left = saved_margin;
            ctx.content_width = ctx.page_width - ctx.margin_left - ctx.margin_right;
            ctx.reset_x();
        ...
        NodeType::ThematicBreak => {
            let rule_y = ctx.y - Fp266::from_int(6);
            let rule_width = ctx.page_width - ctx.margin_left - ctx.margin_right;
            emit_helpers::emit_draw_rule(page, ctx.margin_left, rule_y, rule_width, Fp266::from_frac(1, 2));
            ctx.y = rule_y - Fp266::from_int(6);
            ctx.reset_x();
            maybe_new_page(page, ctx, gir_doc);
        }
        ...
        NodeType::Link { url, .. } => {
            ...
            if !resolved.trim().is_empty() {
                ...
                page.links.push(ldir_ir::gir::GIRLink {
                    ...
                    width: ctx.content_width.to_f64(), height: line_h,
                    url: url.clone(), destination_page: None,
                });
            }
        }
        NodeType::MathInline { content } => {
...
fn compile_children(
    ...
    for &child_id in &node.child_ids {
        compile_v2_node(child_id, module, page, ctx, gir_doc, &mut labels, &refs_map, &mut eq_counter, &mut section_counters, &mut section_number)?;
    }
    Ok(())
}
...
fn compile_children_with_refs(
    ...
    for &child_id in &node.child_ids {
        compile_v2_node(child_id, module, page, ctx, gir_doc, labels, refs_map, eq_counter, section_counters, section_number)?;
    }
    Ok(())
}
...
fn compile_children_with_bib(
    ...
    for &child_id in &node.child_ids {
        compile_v2_node_with_bib(child_id, module, page, ctx, gir_doc, labels, refs_map, eq_counter, section_counters, section_number, bibliography, cite_counter, cite_numbers)?;
    }
    Ok(())
}
...
fn collect_v2_labels(module: &SIRModuleV2) -> HashMap<String, String> {
    ...
    for (label, info) in &module.annotations.labels {
        if !labels.contains_key(label) {
            labels.insert(label.clone(), String::new());
            match info.category {
                LabelCategory::Section => { labels.insert(label.clone(), String::new()); }
                LabelCategory::Equation => { labels.insert(label.clone(), String::new()); }
                LabelCategory::Figure => { labels.insert(label.clone(), String::new()); }
                _ => {}
            }
        }
    }
...
fn collect_v2_refs(module: &SIRModuleV2, labels: &HashMap<String, String>) -> HashMap<String, String> {
...
fn emit_v2_heading_with_number(
    ...
    let text = module.body.collect_text(node_id);
    let numbered_text = if number.is_empty() {
        text
    } else {
        format!("{} {}", number, text)
    };
    if !numbered_text.trim().is_empty() {
        emit_v2_paragraph_text(&numbered_text, page, ctx, gir_doc);
    }
...
fn emit_bibliography_page(
    ...
    let heading_text = "References";
    let saved_font_size = ctx.font_size;
    ctx.font_size = Fp266::from_int(20);
    emit_v2_paragraph_text(heading_text, page, ctx, gir_doc);
    ctx.font_size = saved_font_size;
    ctx.advance_y(ctx.line_height());
    ctx.reset_x();
...
fn emit_v2_math_block(
    ...
    let result = crate::compiler::math::layout_math(
        math_text,
        font_bytes,
        ctx.font_size,
        ctx.y,
    ...
    for glyph in &result.glyphs {
        ...
        if glyph.glyph_id == -1 {
            ...
            page.push(GIRCommand::new_put_glyph(glyph.glyph_id, glyph.advance.raw() as i32));
        }
    }
...
fn compile_v2_table_fallback(
    ...
    for &child_id in &node.child_ids {
        ...
        if !matches!(child.node_type, NodeType::TableRow { .. }) {
            continue;
        }
    ...
    let col_widths_pt: Vec<f64> = {
        let total_text_width: usize = col_widths.iter().sum::<usize>() + col_widths.len() * 3;
        if total_text_width as f64 > content_width_pts {
    ...
    for (row_idx, row) in rows.iter().enumerate() {
        ...
        for (col_idx, cell) in row.iter().enumerate() {
            ...
            emit_helpers::emit_move_xy(page, cx, ctx.y);
...
use ldir_ir::sir::v2::nodes::{Node, NodeType};
    ...
    fn make_simple_v2_module() -> SIRModuleV2 {
        ...
        let para_id = module.body.push(Node::new(1, NodeType::Paragraph).with_parent(doc_id));
        let text_id = module.body.push(Node::new(2, NodeType::Text { content: "Hello world".into() }).with_parent(para_id));
        if let Some(p) = module.body.get_mut(doc_id) { p.add_child(para_id); }
        if let Some(p) = module.body.get_mut(para_id) { p.add_child(text_id); }
        module
    }
    ...
    fn test_v2_compile_heading() {
        let mut module = SIRModuleV2::new();
        let doc_id = module.body.push(Node::new(0, NodeType::Document));
        let ch_id = module.body.push(Node::new(1, NodeType::Chapter).with_parent(doc_id));
        let text_id = module.body.push(Node::new(2, NodeType::Text { content: "Introduction".into() }).with_parent(ch_id));
        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(ch_id); }
        if let Some(c) = module.body.get_mut(ch_id) { c.add_child(text_id); }
    ...
    fn test_v2_compile_bold_italic() {
        ...
        let doc_id = module.body.push(Node::new(0, NodeType::Document));
        let para_id = module.body.push(Node::new(1, NodeType::Paragraph).with_parent(doc_id));
        let bold_id = module.body.push(Node::new(2, NodeType::Bold).with_parent(para_id));
        let text_id = module.body.push(Node::new(3, NodeType::Text { content: "bold text".into() }).with_parent(bold_id));
        let italic_id = module.body.push(Node::new(4, NodeType::Italic).with_parent(para_id));
        let text2_id = module.body.push(Node::new(5, NodeType::Text { content: " italic text".into() }).with_parent(italic_id));
        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(para_id); }
        if let Some(p) = module.body.get_mut(para_id) { p.add_child(bold_id); p.add_child(italic_id); }
        if let Some(b) = module.body.get_mut(bold_id) { b.add_child(text_id); }
        if let Some(i) = module.body.get_mut(italic_id) { i.add_child(text2_id); }
    ...
    fn test_v2_compile_thematic_break() {
        ...
        let doc_id = module.body.push(Node::new(0, NodeType::Document));
        let para_id = module.body.push(Node::new(1, NodeType::Paragraph).with_parent(doc_id));
        let text_id = module.body.push(Node::new(2, NodeType::Text { content: "Before".into() }).with_parent(para_id));
        let break_id = module.body.push(Node::new(3, NodeType::ThematicBreak).with_parent(doc_id));
        let para2_id = module.body.push(Node::new(4, NodeType::Paragraph).with_parent(doc_id));
        let text2_id = module.body.push(Node::new(5, NodeType::Text { content: "After".into() }).with_parent(para2_id));
        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(para_id); d.add_child(break_id); d.add_child(para2_id); }
        if let Some(p) = module.body.get_mut(para_id) { p.add_child(text_id); }
        if let Some(p) = module.body.get_mut(para2_id) { p.add_child(text2_id); }
    ...
    fn test_v2_compile_blockquote() {
        let mut module = SIRModuleV2::new();
        let doc_id = module.body.push(Node::new(0, NodeType::Document));
        let bq_id = module.body.push(Node::new(1, NodeType::BlockQuote).with_parent(doc_id));
        let para_id = module.body.push(Node::new(2, NodeType::Paragraph).with_parent(bq_id));
        let text_id = module.body.push(Node::new(3, NodeType::Text { content: "A quoted passage.".into() }).with_parent(para_id));
        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(bq_id); }
        if let Some(b) = module.body.get_mut(bq_id) { b.add_child(para_id); }
        if let Some(p) = module.body.get_mut(para_id) { p.add_child(text_id); }
    ...
    fn test_v2_compile_page_break() {
        let mut module = SIRModuleV2::new();
        let doc_id = module.body.push(Node::new(0, NodeType::Document));
        let para1_id = module.body.push(Node::new(1, NodeType::Paragraph).with_parent(doc_id));
        let text1_id = module.body.push(Node::new(2, NodeType::Text { content: "Page one content.".into() }).with_parent(para1_id));
        let pb_id = module.body.push(Node::new(3, NodeType::PageBreak).with_parent(doc_id));
        let para2_id = module.body.push(Node::new(4, NodeType::Paragraph).with_parent(doc_id));
        let text2_id = module.body.push(Node::new(5, NodeType::Text { content: "Page two content.".into() }).with_parent(para2_id));
        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(para1_id); d.add_child(pb_id); d.add_child(para2_id); }
        if let Some(p) = module.body.get_mut(para1_id) { p.add_child(text1_id); }
        if let Some(p) = module.body.get_mut(para2_id) { p.add_child(text2_id); }
        ...
        assert!(gir.is_well_formed());
        assert!(gir.page_count() >= 2, "page break should create at least 2 pages");
    }
    ...
    fn test_v2_compile_multiline_paragraph() {
        ...
        let doc_id = module.body.push(Node::new(0, NodeType::Document));
        let para_id = module.body.push(Node::new(1, NodeType::Paragraph).with_parent(doc_id));
        let text_id = module.body.push(Node::new(2, NodeType::Text { content: long_text.into() }).with_parent(para_id));
        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(para_id); }
        if let Some(p) = module.body.get_mut(para_id) { p.add_child(text_id); }
    ...
    fn test_v2_compile_multiple_headings() {
        ...
        let ch_id = module.body.push(Node::new(1, NodeType::Chapter).with_parent(doc_id));
        let ch_text = module.body.push(Node::new(2, NodeType::Text { content: "Chapter 1".into() }).with_parent(ch_id));
        if let Some(c) = module.body.get_mut(ch_id) { c.add_child(ch_text); }

        let sec_id = module.body.push(Node::new(3, NodeType::Section).with_parent(doc_id));
        let sec_text = module.body.push(Node::new(4, NodeType::Text { content: "Section 1.1".into() }).with_parent(sec_id));
        if let Some(s) = module.body.get_mut(sec_id) { s.add_child(sec_text); }

        let para_id = module.body.push(Node::new(5, NodeType::Paragraph).with_parent(doc_id));
        let para_text = module.body.push(Node::new(6, NodeType::Text { content: "Some content here.".into() }).with_parent(para_id));
        if let Some(p) = module.body.get_mut(para_id) { p.add_child(para_text); }

        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(ch_id); d.add_child(sec_id); d.add_child(para_id); }
    ...
    fn test_v2_compile_code_block() {
        let mut module = SIRModuleV2::new();
        let doc_id = module.body.push(Node::new(0, NodeType::Document));
        let code_id = module.body.push(Node::new(1, NodeType::CodeBlock { language: Some("rust".into()) }).with_parent(doc_id));
        let text_id = module.body.push(Node::new(2, NodeType::Text { content: "fn main() { println!(\"Hello\"); }".into() }).with_parent(code_id));
        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(code_id); }
        if let Some(c) = module.body.get_mut(code_id) { c.add_child(text_id); }
    ...
    fn test_v2_compile_list_fallback() {
        let mut module = SIRModuleV2::new();
        let doc_id = module.body.push(Node::new(0, NodeType::Document));
        let list_id = module.body.push(Node::new(1, NodeType::List {
            list_type: ldir_ir::sir::v2::nodes::ListType::Unordered,
            ordered: false,
            start: None,
        }).with_parent(doc_id));

        let item1_id = module.body.push(Node::new(2, NodeType::ListItem).with_parent(list_id));
        let item1_text = module.body.push(Node::new(3, NodeType::Text { content: "First item".into() }).with_parent(item1_id));
        if let Some(i) = module.body.get_mut(item1_id) { i.add_child(item1_text); }

        let item2_id = module.body.push(Node::new(4, NodeType::ListItem).with_parent(list_id));
        let item2_text = module.body.push(Node::new(5, NodeType::Text { content: "Second item".into() }).with_parent(item2_id));
        if let Some(i) = module.body.get_mut(item2_id) { i.add_child(item2_text); }

        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(list_id); }
        if let Some(l) = module.body.get_mut(list_id) { l.add_child(item1_id); l.add_child(item2_id); }
    ...
    fn test_v2_compile_ordered_list() {
        ...
        let item1_id = module.body.push(Node::new(2, NodeType::ListItem).with_parent(list_id));
        let item1_text = module.body.push(Node::new(3, NodeType::Text { content: "First".into() }).with_parent(item1_id));
        if let Some(i) = module.body.get_mut(item1_id) { i.add_child(item1_text); }

        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(list_id); }
        if let Some(l) = module.body.get_mut(list_id) { l.add_child(item1_id); }
    ...
    fn test_v2_compile_link() {
        ...
        let doc_id = module.body.push(Node::new(0, NodeType::Document));
        let para_id = module.body.push(Node::new(1, NodeType::Paragraph).with_parent(doc_id));
        let link_id = module.body.push(Node::new(2, NodeType::Link {
            url: "https://example.com".into(),
            title: None,
        }).with_parent(para_id));
        let text_id = module.body.push(Node::new(3, NodeType::Text { content: "click here".into() }).with_parent(link_id));
        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(para_id); }
        if let Some(p) = module.body.get_mut(para_id) { p.add_child(link_id); }
        if let Some(l) = module.body.get_mut(link_id) { l.add_child(text_id); }
    ...
    fn test_v2_compile_mixed_formatting() {
        ...
        let doc_id = module.body.push(Node::new(0, NodeType::Document));
        let para_id = module.body.push(Node::new(1, NodeType::Paragraph).with_parent(doc_id));
        let text1 = module.body.push(Node::new(2, NodeType::Text { content: "Normal ".into() }).with_parent(para_id));
        let bold = module.body.push(Node::new(3, NodeType::Bold).with_parent(para_id));
        let text2 = module.body.push(Node::new(4, NodeType::Text { content: "bold".into() }).with_parent(bold));
        let italic = module.body.push(Node::new(5, NodeType::Italic).with_parent(para_id));
        let text3 = module.body.push(Node::new(6, NodeType::Text { content: " italic".into() }).with_parent(italic));
        let mono = module.body.push(Node::new(7, NodeType::Mono).with_parent(para_id));
        let text4 = module.body.push(Node::new(8, NodeType::Text { content: " code".into() }).with_parent(mono));
        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(para_id); }
        if let Some(p) = module.body.get_mut(para_id) { p.add_child(text1); p.add_child(bold); p.add_child(italic); p.add_child(mono); }
        if let Some(b) = module.body.get_mut(bold) { b.add_child(text2); }
        if let Some(i) = module.body.get_mut(italic) { i.add_child(text3); }
        if let Some(m) = module.body.get_mut(mono) { m.add_child(text4); }
    ...
    fn test_collect_v2_labels_from_module() {
        let mut module = SIRModuleV2::new();
        let doc_id = module.body.push(Node::new(0, NodeType::Document));
        let sec_id = module.body.push(Node::new(1, NodeType::Section).with_label("sec:intro").with_parent(doc_id));
        let sec_text = module.body.push(Node::new(2, NodeType::Text { content: "Introduction".into() }).with_parent(sec_id));
        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(sec_id); }
        if let Some(s) = module.body.get_mut(sec_id) { s.add_child(sec_text); }
    ...
    fn test_collect_v2_labels_from_annotations() {
        let mut module = SIRModuleV2::new();
        module.annotations.add_label("fig:diagram".to_string(), 42, LabelCategory::Figure);
    ...
    fn test_section_numbering() {
        ...
        let sec1 = module.body.push(Node::new(1, NodeType::Section).with_parent(doc_id));
        let sec1_text = module.body.push(Node::new(2, NodeType::Text { content: "First Section".into() }).with_parent(sec1));
        if let Some(s) = module.body.get_mut(sec1) { s.add_child(sec1_text); }

        let sec2 = module.body.push(Node::new(3, NodeType::Section).with_parent(doc_id));
        let sec2_text = module.body.push(Node::new(4, NodeType::Text { content: "Second Section".into() }).with_parent(sec2));
        if let Some(s) = module.body.get_mut(sec2) { s.add_child(sec2_text); }

        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(sec1); d.add_child(sec2); }
    ...
    fn test_v2_compile_citation_node() {
        ...
        let doc_id = module.body.push(Node::new(0, NodeType::Document));
        let para_id = module.body.push(Node::new(1, NodeType::Paragraph).with_parent(doc_id));
        let text1 = module.body.push(Node::new(2, NodeType::Text { content: "As shown in ".into() }).with_parent(para_id));
        let cite = module.body.push(Node::new(3, NodeType::Citation {
            keys: vec!["knuth1984".to_string()],
            style: None,
        }).with_parent(para_id));
        let text2 = module.body.push(Node::new(4, NodeType::Text { content: ", we see...".into() }).with_parent(para_id));
        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(para_id); }
        if let Some(p) = module.body.get_mut(para_id) { p.add_child(text1); p.add_child(cite); p.add_child(text2); }
    ...
    fn test_v2_compile_citation_with_bib() {
        ...
        let doc_id = module.body.push(Node::new(0, NodeType::Document));
        let para_id = module.body.push(Node::new(1, NodeType::Paragraph).with_parent(doc_id));
        let text1 = module.body.push(Node::new(2, NodeType::Text { content: "See ".into() }).with_parent(para_id));
        let cite = module.body.push(Node::new(3, NodeType::Citation {
            keys: vec!["knuth1984".to_string()],
            style: None,
        }).with_parent(para_id));
        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(para_id); }
        if let Some(p) = module.body.get_mut(para_id) { p.add_child(text1); p.add_child(cite); }
        ...
        let bibliography = crate::compiler::bibtex::parse_bib(bib_content).expect("parse should succeed");
    ...
    fn test_v2_compile_with_refs_and_labels() {
        ...
        let sec_id = module.body.push(Node::new(1, NodeType::Section).with_label("sec:test").with_parent(doc_id));
        let sec_text = module.body.push(Node::new(2, NodeType::Text { content: "Test Section".into() }).with_parent(sec_id));
        if let Some(s) = module.body.get_mut(sec_id) { s.add_child(sec_text); }

        let para_id = module.body.push(Node::new(3, NodeType::Paragraph).with_parent(doc_id));
        let para_text = module.body.push(Node::new(4, NodeType::Text { content: r"Refer to \ref{sec:test} here.".into() }).with_parent(para_id));
        if let Some(p) = module.body.get_mut(para_id) { p.add_child(para_text); }

        if let Some(d) = module.body.get_mut(doc_id) { d.add_child(sec_id); d.add_child(para_id); }
