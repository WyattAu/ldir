...
Register-ArgumentCompleter -Native -CommandName 'ldc' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'ldc' {
            ...
            [CompletionResult]::new('--ot-features', '--ot-features', [CompletionResultType]::ParameterName, 'OpenType features (e.g., "kern,liga,dlig,hlig"). Prefix with - to disable: "kern,-liga". Default: HarfBuzz defaults')
            [CompletionResult]::new('--config', '--config', [CompletionResultType]::ParameterName, 'Path to configuration file. Default: ./ldir.toml')
            ...
            [CompletionResult]::new('--no-config', '--no-config', [CompletionResultType]::ParameterName, 'Skip loading any configuration file')
            [CompletionResult]::new('--dump-config', '--dump-config', [CompletionResultType]::ParameterName, 'Print the effective (merged) configuration as TOML and exit')
